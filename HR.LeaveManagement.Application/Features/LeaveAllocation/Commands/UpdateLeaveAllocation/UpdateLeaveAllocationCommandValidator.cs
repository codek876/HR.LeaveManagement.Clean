        ...
        AbstractValidator<UpdateLeaveAllocationCommand>
    {
            ...
            ILeaveAllocationRepository leaveAllocationRepository)
        {
            ...
            RuleFor(p => p.NumberOfDays)
                .GreaterThan(0).WithMessage("{PropertyName} must be greater than" +
                "{ComparisonValue}");

            RuleFor(p => p.Period)
                .GreaterThanOrEqualTo(DateTime.Now.Year).WithMessage("{PropertyName} must be after" +
                "{ComparisonValue}");
        ...
        {
            var allocation = await _leaveAllocationRepository.GetByIdAsync(id);
